    ...
    while lines:
        ...
        if l.lower().startswith("*partition"):
            ...
            for i in range(int(nnodes)):
                l = next(lines)
                key = str(i+1)
                partition[key] = int(l.strip())
        elif l.lower().startswith("*network"):
            try:
        ...
        # biomass information
        elif l.lower().startswith("*vector"):
            biomass = {}
            l = next(lines)
            l, nnodes = l.split()
            for i in range(int(nnodes)):
                l = next(lines)
                key = str(i+1)
                biomass[key] = float(l.strip())
    # add biomass and trophic level information
    for i in biomass:
        label = nodelabels[i]
        G.nodes[label]["biomass"] = biomass[i]
    for i in partition:
        label = nodelabels[i]
        G.nodes[label]["trophic_level"] = partition[i]
